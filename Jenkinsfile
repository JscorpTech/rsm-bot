pipeline {
    ...
    environment {
        PROD_ENV     = "/opt/env/.env.rsm"
        IMAGE_NAME   = "rsm"
        TEST_TAG     = "test"
        PROD_TAG     = "latest"
        CONTAINER_DB = "rsm_db_test"
        CONTAINER_WEB = "rsm_web_test"
        CONTAINER_REDIS = "rsm_redis_test"
        STACK_NAME = "rsm"
    }
    ...
    stages {
        stage('Checkout Code') {
            steps {
                git branch: 'main', credentialsId: 'ssh', url: 'git@github.com:JscorpTech/rsm.git'
